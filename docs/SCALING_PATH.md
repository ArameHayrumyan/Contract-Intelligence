...
| **Database / persistence** | SQLite via SQLAlchemy Core by default; **PostgreSQL supported today** by setting `DATABASE_URL` (drivers in the `[postgres]` extra). Covers the document/standard registry, audit results, annotations, and the activity log | Point `DATABASE_URL` at managed PostgreSQL (RDS / Cloud SQL) — **no code change**; async (`asyncpg`) and sync (`psycopg`) driver variants are derived automatically | `packages/rag_core/rag_core/{database,registry_store,config}.py`. Using Core (not the ORM) keeps the schema readable and the swap a connection-string change |
...
| **Compliance** | Append-only `activity_log` + persisted audit/cross-reference results (SQLite/Postgres). Every audit, cross-reference, status change, annotation, and export is recorded; a static test enforces no `UPDATE`/`DELETE` against the log | Write-once / immutable store (AWS CloudTrail, Azure Monitor, or a Postgres trigger blocking mutations) | `packages/rag_core/rag_core/database.py` (the constraint is architectural — no mutation path exists) |
